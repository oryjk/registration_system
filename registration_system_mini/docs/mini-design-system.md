...
- 多场待处理比赛使用普通横向 `swiper`，默认显示第一场，多场时每 5 秒自动循环切换；手指触摸、展开人员名单、离开页面或打开比赛期间暂停自动轮播；保留卡片右上角的进度点（当前场次为长点）及标题右侧左右按钮，不在标题旁重复显示数字序号。单场只显示该卡，不伪造底卡。左右按钮只按当前位置禁用：首场禁用左侧、末场禁用右侧，中间均可用；滑动期间不切换按钮的颜色或透明度，仅在事件处理时拦截重复操作。
...
球队资料、队员管理和队员出勤使用页面内 `TeamManagePanel` 统一轻描边表面、标题与内边距；布局由本地 wrapper 承担，不依赖父级 scoped 穿透组件。资料、入队密码与低频解散操作分组；队员按管理角色、普通队员、冻结队员分组，保留姓名和角色；出勤、编辑、冻结／恢复、移除合并为同一排带图标的紧凑胶囊按钮，尺寸及浅底风格统一，移除保留浅危险语义色。比赛出勤展开后使用 success/warning/neutral 的前景和背景对展示统计及状态，姓名、比赛名和地点允许换行。保持原有权限、提交与确认流程。
