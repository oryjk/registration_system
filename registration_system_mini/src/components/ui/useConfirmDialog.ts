...
import type { ConfirmDialogTone } from "./ConfirmDialog.vue";
...
export interface ConfirmDialogOptions {
  ...
  links?: ConfirmDialogLinkOptions[];
  /** 危险操作（取消报名等）主按钮使用 danger 色调。 */
  danger?: boolean;
}

export interface ConfirmDialogLinkOptions {
  text: string;
  onTap: () => void;
}
...
/**
 * 统一风格确认弹窗的状态驱动 composable。
 *
 * 页面在模板中渲染 <ConfirmDialog> 并绑定返回的 visible/state/handlers，
 * 业务逻辑通过 confirm() 以 Promise 方式等待用户选择，替代 uni.showModal。
 */
export function useConfirmDialog() {
  const confirmDialogVisible = ref(false);
  const confirmDialogState = reactive<ConfirmDialogState>({
    title: "",
    message: "",
    imageSrc: "",
    imageCaption: "",
  ...
  function confirm(options: ConfirmDialogOptions): Promise<boolean> {
    ...
    confirmDialogState.primaryIcon = options.confirmIcon ?? "";
    confirmDialogState.secondaryIcon = options.cancelIcon ?? "";
