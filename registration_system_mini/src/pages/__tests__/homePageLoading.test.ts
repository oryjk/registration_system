...
describe("home page loading states", () => {
  ...
  test("refreshes the others-tab square list on pull-down in parallel with my matches", async () => {
    const homePageSource = await sourceFile(
      "pages/home/index.vue",
    ).text();

    expect(homePageSource.includes('activeHomeTab.value === "others" && !isGuestMode.value ? otherMatches.loadPage() : null')).toEqual(true);
    expect(homePageSource.includes("await loadPageData({ preserveContent: hasLoadedOnce.value });")).toEqual(true);
