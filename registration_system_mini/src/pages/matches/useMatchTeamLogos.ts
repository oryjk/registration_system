...
export function useMatchTeamLogos(options: {
  ...
  const awayTeamLogoUrl = computed(() => {
    ...
    if (typeof awayTeamId !== "number") return "";
    const awayTeam = teamsById.value[awayTeamId];
    const matchingOpponent = opponentTeam.value?.id === awayTeamId ? opponentTeam.value : null;
    return (awayTeam ?? matchingOpponent)?.logo_url?.trim() || "";
  });
