...
Go 后端 + 小程序 H5 + Go 管理端由仓库根目录的 `deploy_jd_go_h5.sh` 部署。前端本地构建，Go 提交快照上传 jd 构建镜像，通过现有 Compose 更新 backend；健康检查失败恢复旧镜像。环境文件为 jd 的 `/root/docker_data/registration-go-v3/backend.env`，本地上传目录与 MinIO 持久化目录保持不变。数据库前向迁移需显式加 `--migrate`，不会随普通部署自动执行。详见 [jd 部署文档](../docs/deployment-jd.md)。
