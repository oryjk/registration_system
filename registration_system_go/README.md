...
退款、提现、人工调账、微信内 H5 支付、签到和通知暂不实现。**Rust 后端（`../registration_system_rs/`）已冻结、未来不再更新**，仅作为业务与迁移的只读参考；旧余额、订单和流水不迁移到 Go，所有新后端开发都在本项目进行。历史比赛与报名数据已通过一键迁移脚本（见「一键迁移旧库数据」）从 Rust 旧库迁入 Go 结构。

## 部署（out109 验收环境）

Go 后端 + 小程序 H5 + Go 管理端统一由仓库根目录的 `deploy_out109_go_h5.sh` 部署（本地构建前端产物、远端构建 Docker 镜像、发布端口 127.0.0.1:18081、健康检查失败自动回滚）。手动步骤只在排查时使用。
