...
- 已新增 `payment` 与 `wallet` 六边形模块：微信支付 V2 小程序 JSAPI 支持统一下单、查单、关单、签名验签和结果通知。
- 支付结果通过单 PostgreSQL 事务完成订单置为 paid、唯一充值流水和钱包余额更新；重复通知、重复同步和并发结算不会重复充值。
- 已新增 App 充值订单/钱包接口、Admin 只读支付/钱包接口和独立 `/api/v1/webhooks/wechat-pay` 回调；全部路径带 v1 且区分调用方。
- development/test Mock 创建订单后保持 pending，首次同步模拟成功；production 禁止 Mock 且缺少 V2 商户或公网回调配置时拒绝启动。
- Go 钱包从零开始，不迁移或修改 Rust 余额、订单和流水；首版不支持退款、提现、人工调账或微信内 H5 支付。
- 合并前复审已修复 migration 版本冲突、来源键异常导致的漏充值、钱包结算时间倒退、非法支付状态筛选和微信查单订单号关联问题，并补充迁移唯一性与资金并发/回滚测试。
