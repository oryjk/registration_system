...
新的赛事报名与球队管理后端，**当前唯一在开发的后端**。已覆盖微信登录、用户/球队权限、Match 比赛、支付钱包、队费结算与通知。旧 Rust 项目 `../registration_system_rs/` 已停止开发并从工作区删除（2026-08-30，需要核对旧实现时从 git 历史检出只读查看）；不要重建或修改它。
...
- 已实现站内通知与管理员队费入账；当前无通用人工调账、退款、提现、微信内 H5 支付和签到接口，不要将已有入账能力扩展为任意余额修改。
