...
func mapAdminDetailMatch(row matchsqlc.GetMatchForAdminRow) domain.Match {
	return domain.Match{
		...
		PlayersPerTeam: int(row.PlayersPerTeam), HostScore: intPointer(row.HostScore), AwayScore: intPointer(row.AwayScore),
		...
		IsFree: row.IsFree, PaymentMode: domain.PaymentMode(row.PaymentMode), FeePerPersonCents: row.FeePerPersonCents, FeeType: domain.FeeType(row.FeeType),
