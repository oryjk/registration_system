...
export default function DashboardPage() {
  ...
  const checkHealth = useCallback(async (shouldApply = alwaysApply) => {
    ...
    if (shouldApply())
      setSnapshot((current) => ({ ...current, state: "checking", error: "" }));
    try {
      await getHealth();
    ...
    <main className="dashboard-page">
      ...
      <section className="status-strip" aria-label="服务状态摘要">
        ...
        <div className="status-cell">
          <Text>响应时间</Text>
          <strong>
            {snapshot.latency === null ? "--" : `${snapshot.latency} ms`}
          </strong>
        </div>
        <div className="status-cell">
          <Text>最近检查</Text>
      ...
      <div className="dashboard-grid">
        <section className="data-panel">
          ...
          <Descriptions column={1} size="small" colon={false}>
            <Descriptions.Item label="服务地址">
              <Text copyable>{getApiBaseUrl()}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="健康路由">
              <Text code>/health</Text>
            </Descriptions.Item>
            <Descriptions.Item label="管理前缀">
        ...
        <section className="data-panel panel-note">
          ...
          <Text className="panel-paragraph">
            管理员认证、球队与比赛管理路由已接入，业务请求使用 Bearer JWT 鉴权。
          </Text>
        </section>
      </div>
          ...
          columns={[
            {
              ...
              width: compactTable ? 90 : 150,
            },
            {
              title: "路由",
              dataIndex: "route",
              render: (value: string) => (
                <Text code className="route-code">
                  {value}
                </Text>
              ),
            },
            ...(compactTable
              ? []
              : [{ title: "当前层级", dataIndex: "layer", width: 150 }]),
            {
              title: "状态",
              dataIndex: "state",
              ...
              render: (value: string) => {
                ...
                if (value === "handler")
                  return <Tag color="processing">待路由装配</Tag>;
                return <Tag>待实现</Tag>;
              },
            },
          ]}
