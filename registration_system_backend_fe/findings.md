...
- 当前管理端只有 `/players` 球员管理页，且仅通过 `Player.is_venue` badge 和编辑开关暴露场馆身份，没有运营友好的独立场馆管理入口。
- 登录页已经走账号密码登录后台管理员，不受本轮场馆用户密码登录影响；场馆账号登录约束主要影响小程序/用户端登录链路。
- 现有 `src/services/player.ts` 已覆盖球员列表、创建、编辑、冻结/解冻，适合作为新增 `venue.ts` 的参考，但场馆管理需要额外的“按昵称搜索绑定”和“独立账号创建/改密”能力。
- Layout 导航当前已有“球员”但没有“场馆”入口，本轮需要补充独立页面而不是继续堆在 `PlayerList.vue`。
- 继续往 `PlayerList.vue` 里叠场馆专用弹窗会让页面职责失控；更稳的做法是新建 `views/venues/VenueList.vue`，复用服务和交互模式，但保持页面边界清楚。
- 场馆来源识别不能靠前端猜测昵称或手机号；依赖后端返回的 `username` 更稳，`username` 存在时可视为独立账号。
