...
赛事报名与球队管理 monorepo，当前由 **6 个**相互协作的子项目组成。

工作区正在从 **Rust 老后端** 迁移到 **Go 新后端**：Go 后端在复刻并优化 Rust 后端的业务模型，管理端保留新旧两个项目，小程序则统一在 `registration_system_mini/` 内原地切换，不再维护平行的 Go 版本。
