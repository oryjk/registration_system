...
赛事报名与球队管理 monorepo，当前由 **7 个**相互协作的子项目组成。

工作区正在从 **Rust 老后端** 迁移到 **Go 新后端**：Go 后端在复刻并优化 Rust 后端的业务模型，各端也分化为「对接 Rust 的老版」和「对接 Go 的新版」两条链路。子项目表、联调顺序和接口前缀都以这两条链路为准。
...
| `registration_system_mini/` | 用户侧小程序（**老版**，已接入主要真实接口） | `uni-app + Vue 3 + TypeScript + Vite` | **Rust** |
| `registration_system_mini_go/` | 对接 Go 新后端的用户端（**新版**，脚手架/早期阶段） | `uni-app + Vue 3 + TypeScript + Vite + Bun` | **Go** |
...
> 阶段说明：Rust 后端已完整可用但**不再增加代码**；Go 后端在第一阶段，已闭环覆盖微信登录 + JWT 鉴权 + 管理员体系、球队与成员管理、比赛三种发布模式与整队约队闭环、用户资料维护，但订单/支付/账单/结算/签到/通知**不在第一阶段**。两个新版前端按 Go 后端的落地节奏逐步推进，成熟度低于对应的老版。

## 文档入口

- 工作区协作约定：`AGENTS.md`
- 小程序（老版）说明：`registration_system_mini/README.md`
- 小程序（新版）说明：`registration_system_mini_go/README.md`
- 管理后台（老版）说明：`registration_system_backend_fe/README.md`
...
- 本地开发端口默认 `5373`。
- `VITE_API_BASE_URL` 默认空串 → axios baseURL 退化为相对路径 `/api/admin`，依赖 Vite dev proxy `/api → http://localhost:18080`。
- 生产环境需由 nginx 把 `/api/admin/*` 转发到 Rust 后端，生产子路径 `/regist-admin/`。
...
```bash
cd registration_system_mini_go
bun install
bun run dev:h5         # 或 bun run dev:mp-weixin
```

- 默认 baseURL `http://127.0.0.1:18080`（见 `.env.example`）；`dev:h5` 用 `/go-api` + Vite proxy。
- 请求层**自己加 `/api`**（与老版相反）；响应契约 `{ code, message, data }`，成功 `code === 0`。
- ⚠️ **当前处于脚手架/早期阶段**：只有 1 个 home 验证页（登录 + 球队列表 + 健康检查），尚无比赛、约队、账单等业务页面。
...
- 改 Rust 后端 → `registration_system_backend_fe/src/services/`、`registration_system_mini/src/api/`
- 改 Go 后端 → `registration_system_backend_fe_go/src/api/`、`registration_system_mini_go/src/api/`
...
- **老版 Vue 管理后台**：登录、仪表盘、球队、活动、约队、账单、球员、场馆、管理员、系统设置等模块已完整落地，处于存量维护/待替代阶段。
- **新版 React 管理后台**：登录、鉴权、仪表盘、比赛（CRUD + 状态流转）、球队（含成员/队长/球员资料）、管理员、接入状态已落地，覆盖约老版 3/8 业务域；billing/challenges/venues 尚未开始。
- **老版小程序**：首页、约队、比赛详情、球队管理、账单、通知、个人中心等已接入 Rust 真实接口；`src/mock/` 仅作历史原型留存。
- **新版小程序**：脚手架阶段，仅 home 验证页（登录 + 球队列表 + 健康检查）。
- **移动管理 App**：已初始化 Flutter iOS/Android 项目，提供登录 + 工作台 + 创建比赛 + 创建球队首版页面。
