...
use crate::team::adapters::{PostgresTeamRepository, create_admin_router, create_app_router};
...
pub fn build_team_service(pool: &PgPool) -> Arc<TeamService> {
    let repository = Arc::new(PostgresTeamRepository::new(pool.clone()));
    let activity_repository = Arc::new(PostgresActivityRepository::new(pool.clone()));
    Arc::new(TeamService::new(repository, activity_repository))
}
