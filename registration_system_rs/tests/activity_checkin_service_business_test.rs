...
struct InMemoryActivityRepository {
    ...
    configs: Mutex<HashMap<(String, String), ActivityTeamCheckInConfig>>,
    checkins: Mutex<HashMap<(String, String, i64), ActivityCheckInRecord>>,
}
...
impl ActivityRepository for InMemoryActivityRepository {
    ...
    async fn upsert_team_checkin_config(
        ...
        self.configs.lock().unwrap().insert(
            (config.activity_id.clone(), config.team_id.clone()),
            config.clone(),
        );
        Ok(())
    }
    ...
    async fn find_team_checkin_config(
        ...
        Ok(self
            ...
            .lock()
            .unwrap()
            .get(&(activity_id.to_string(), team_id.to_string()))
            .cloned())
    }
    ...
    async fn find_checkin_record(
        ...
        Ok(self
            ...
            .lock()
            .unwrap()
            .get(&(activity_id.to_string(), team_id.to_string(), user_id))
            .cloned())
    }
}
...
struct InMemoryTeamAccessPort {
    roles: Mutex<HashMap<(String, i64), String>>,
}
...
fn sample_activity() -> Activity {
    ...
    Activity {
        ...
        status: 0,
        description: None,
        home_team_id: Some("team-home".to_string()),
        away_team_id: Some("team-away".to_string()),
        color: None,
        opposing_color: None,
    ...
    *repository.activity.lock().unwrap() = Some(activity);
    ...
    for (team_id, user_id, role) in roles {
        team_access
            ...
            .lock()
            .unwrap()
            .insert((team_id.to_string(), *user_id), (*role).to_string());
    }
...
async fn leader_can_enable_checkin_for_participating_team() {
    let (service, repository, _) = build_service(sample_activity(), &[("team-home", 7, "leader")]);
    ...
    let saved = repository
        .find_team_checkin_config("activity-checkin-1", "team-home")
        .await
...
async fn member_cannot_configure_checkin_for_team() {
    let (service, _, _) = build_service(sample_activity(), &[("team-home", 9, "member")]);
...
async fn user_cannot_check_in_outside_configured_radius() {
    let (service, _, _) = build_service(
        sample_activity(),
        &[("team-home", 11, "member"), ("team-home", 7, "captain")],
    );
...
async fn successful_check_in_is_recorded_and_cannot_repeat() {
    let (service, repository, _) = build_service(
        sample_activity(),
        &[("team-home", 13, "member"), ("team-home", 7, "captain")],
    );
    ...
    let saved = repository
        .find_checkin_record("activity-checkin-1", "team-home", 13)
        .await
        .expect("repository lookup should succeed")
...
async fn default_checkin_time_uses_local_time_window() {
    ...
    let (service, _, _) = build_service(
        activity,
        &[("team-home", 15, "member"), ("team-home", 7, "captain")],
    );
