...
use registration_system_backend::billing::ports::BillingRepository;
...
async fn cleanup_activity(pool: &PgPool, activity_id: &str) {
    sqlx::query("DELETE FROM rs_user_billings WHERE game_id = $1")
        .bind(activity_id)
        .execute(pool)
        .await
        .ok();
    sqlx::query("DELETE FROM rs_activity_settlement_batches WHERE activity_id = $1")
        ...
        .await
        .ok();
    sqlx::query("DELETE FROM rs_activity_order WHERE activity_id = $1")
        .bind(activity_id)
        .execute(pool)
        .await
        .ok();
...
async fn settle_activity_expense_bills_attending_users_once() {
    ...
    let billing_count = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM rs_user_billings WHERE game_id = $1 AND billing_type = 'game_fee'",
    )
    .bind(&activity_id)
    .fetch_one(&pool)
    .await
